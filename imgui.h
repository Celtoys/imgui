...
namespace ImGui
{
    ...
    IMGUI_API void        TextColored(const ImVec4& col, const char* fmt, ...);               // shortcut to doing PushStyleColor(ImGuiCol_Text, col); Text(fmt, ...); PopStyleColor();
    IMGUI_API void        TextColoredV(const ImVec4& col, const char* fmt, va_list args);
    IMGUI_API void        TextUnformatted(const char* text, const char* text_end = NULL);     // doesn't require null terminated string if 'text_end' is specified. no copy done to any bounded stack buffer, better for long chunks of text.
    ...
    IMGUI_API bool        IsClipped(const ImVec2& item_size);                                 // to perform coarse clipping on user's side (as an optimisation)
    ...
    IMGUI_API ImVec2      CalcTextSize(const char* text, const char* text_end = NULL, const bool hide_text_after_hash = true);
...
struct ImGuiIO
{
    ...
    // Function
    IMGUI_API void        AddInputCharacter(ImWchar); // Helper to add a new character into InputCharacters[]
...
struct ImGuiTextFilter
{
    ...
    IMGUI_API ImGuiTextFilter();
    void Clear() { InputBuf[0] = 0; Build(); }
    IMGUI_API void Draw(const char* label = "Filter (inc,-exc)", float width = -1.0f);    // Helper calling InputText+Build
    IMGUI_API bool PassFilter(const char* val) const;
...
struct ImDrawList
{
    ...
    IMGUI_API void AddText(ImFont font, float font_size, const ImVec2& pos, ImU32 col, const char* text_begin, const char* text_end);
};
...
#pragma pack(pop)
    ...
    IMGUI_API ImVec2                  CalcTextSizeA(float size, float max_width, const char* text_begin, const char* text_end, const char** remaining = NULL) const;            // utf8
    IMGUI_API ImVec2                  CalcTextSizeW(float size, float max_width, const ImWchar* text_begin, const ImWchar* text_end, const ImWchar** remaining = NULL) const;  // wchar
    IMGUI_API void                    RenderText(float size, ImVec2 pos, ImU32 col, const ImVec4& clip_rect, const char* text_begin, const char* text_end, ImDrawVert*& out_vertices) const;
};
